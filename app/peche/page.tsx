  ...
  return (
    <main className="min-h-[100dvh] bg-transparent text-tx-base px-3 sm:px-6 pt-3 sm:pt-5 pb-10">
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          <div className="h-64 rounded-[22px] border-4 border-brand-border bg-brand-card animate-pulse" />
        ) : !isOwner(user?.id) ? (
          ...
          <div className="max-w-xl mx-auto mt-10 bg-brand-card border-4 border-brand-border rounded-[22px] p-8 text-center shadow-[0_8px_0_#05061A]">
            <h2 className="font-display text-3xl">Accès propriétaire</h2>
            <p className="mt-2 text-sm font-bold text-tx-secondary">La V1 jouable arrive ici.</p>
          </div>
        )}
      </div>
    </main>
