...
import { Package, Backpack, Zap } from 'lucide-react';
...
export default function InventoryPanel({
  ...
  busy: string | null;
  onUse: (itemId: string, name: string) => void;
  compact?: boolean;
}) {
  const now = useNow();
    ...
    <div className="rounded-2xl border-4 border-brand-border bg-brand-card p-4">
      ...
      <div className={cn('grid gap-2', compact ? 'grid-cols-1' : 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-5')}>
        {state.crates.map((crate) => (
          <button
            key={crate.id}
            onClick={() => onUse(crate.id, crate.name)}
            disabled={busy !== null}
            className="relative rounded-xl border-2 border-accent-primary bg-accent-primary/10 p-3 text-left hover:-translate-y-0.5 transition-all focus:outline-none disabled:opacity-50"
          >
            <Package className="h-4 w-4 text-accent-primary mb-1" />
            <div className="font-display font-black text-[11px] leading-tight">{crate.name}</div>
            <div className="text-[10px] font-black text-accent-primary">OUVRIR</div>
